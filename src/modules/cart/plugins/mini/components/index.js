...
const MiniComponent = (props) => {
    ...
    const handlingGoToCheckout = useCallback(() => {
        ...
        if (minimumOrderEnabled && grandTotalValue < minimumOrderAmount) {
            ...
            if (storeConfig.pwacheckout?.enable === '1' && storeConfig.pwacheckout?.version === 'V2' && cartId) {
                getScv2Url({
                    variables: {
                        cartId,
                    },
        ...
        <>
            <CommonsDrawer
                ...
                className={cx('mobile:max-tablet:w-[320px]', 'tablet:max-desktop:w-[396px]', 'desktop:w-[540px]', {
                    ...
                    'mobile:max-tablet:animate-drawer-in-mobile tablet:max-desktop:animate-drawer-in-tablet  desktop:animate-drawer-in-desktop':
                        localOpen,
                    'mobile:max-tablet:animate-drawer-out-mobile tablet:max-desktop:animate-drawer-out-tablet  desktop:animate-drawer-out-desktop':
                        !localOpen,
                    'right-0': !localOpen,
                })}
            >
                        ...
                        >
                            ...
                            <div className={cx('swift-minicart__header--close-action', 'text-right')}>
                                <Button
                                    ...
                                    onClick={handleClose}
                                    icon={<XMarkIcon />}
                                    iconProps={{ className: cx('w-5', 'h-5', 'w-[18px]', 'text-neutral-white') }}
                                    iconPosition="left"
                                    iconOnly
                                    variant="tertiary"
                                />
                        ...
                        {data && data.total_quantity > 0 ? (
                            <div className={cx('absolute', 'left-0', 'min-h-[50px]', 'w-full', 'grid', 'grid-cols-1', 'gap-y-4')}>
                                <div className={cx('sub-total', 'px-6', 'flex', 'flex-row', 'justify-between', 'pt-4')}>
                                    <Typography variant="p-1" className={cx('normal-case text-lg')}>
                                        {t('common:cart:cardTotal')}:
                                    </Typography>
                                    <Typography variant="p-1a" className={cx('normal-case', 'text-neutral-800', 'text-lg')}>
